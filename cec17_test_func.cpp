...
void cec17_test_func_init(int nx, int mx)
{
	...
	if (ini_flag==0)
	{
		...
		free(z_cec2017);
		free(x_bound);
		y_cec2017=(double *)malloc(sizeof(double)  *  nx);
		z_cec2017=(double *)malloc(sizeof(double)  *  nx);
...
void sphere_func (double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag) /* Sphere */
{
	int i;
	f[0] = 0.0;
	sr_func (x, z_cec2017, nx, Os, Mr, 1.0, s_flag, r_flag); /* shift and rotate */	
	for (i=0; i<nx; i++)
	{					
		f[0] += z_cec2017[i]*z_cec2017[i];
	}
...
void ellips_func (double *x, double *f, int nx, double *Os,double *Mr, int s_flag, int r_flag) /* Ellipsoidal */
{
	...
	for (i=0; i<nx; i++)
	{
       f[0] += pow(10.0,6.0*i/(nx-1))*z_cec2017[i]*z_cec2017[i];
	}
}
...
void bent_cigar_func (double *x, double *f, int nx, double *Os,double *Mr, int s_flag, int r_flag) /* Bent_Cigar */
{
	...
	f[0] = z_cec2017[0]*z_cec2017[0];
	for (i=1; i<nx; i++)
	{
		f[0] += pow(10.0,6.0)*z_cec2017[i]*z_cec2017[i];
	}
...
void discus_func (double *x, double *f, int nx, double *Os,double *Mr, int s_flag, int r_flag) /* Discus */
{
    int i;
	sr_func (x, z_cec2017, nx, Os, Mr,1.0, s_flag, r_flag); /* shift and rotate */
	f[0] = pow(10.0,6.0)*z_cec2017[0]*z_cec2017[0];
	for (i=1; i<nx; i++)
	{
		f[0] += z_cec2017[i]*z_cec2017[i];
	}
...
void dif_powers_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Different Powers */
{
	...
	for (i=0; i<nx; i++)
	{
		f[0] += pow(fabs(z_cec2017[i]),2+4*i/(nx-1));
	}
	f[0]=pow(f[0],0.5);
}
...
void rosenbrock_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Rosenbrock's */
{
	...
	double tmp1,tmp2;
	f[0] = 0.0;
	sr_func (x, z_cec2017, nx, Os, Mr, 2.048/100.0, s_flag, r_flag); /* shift and rotate */
	z_cec2017[0] += 1.0;//shift to orgin
	for (i=0; i<nx-1; i++)
	{
		z_cec2017[i+1] += 1.0;//shift to orgin
		tmp1=z_cec2017[i]*z_cec2017[i]-z_cec2017[i+1];
		tmp2=z_cec2017[i]-1.0;
...
void schaffer_F7_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Schwefel's 1.2  */
{
	...
	for (i=0; i<nx-1; i++)	
	{
		z_cec2017[i]=pow(y_cec2017[i]*y_cec2017[i]+y_cec2017[i+1]*y_cec2017[i+1],0.5);
		tmp=sin(50.0*pow(z_cec2017[i],0.2));
		f[0] += pow(z_cec2017[i],0.5)+pow(z_cec2017[i],0.5)*tmp*tmp ;
	}
...
void ackley_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Ackley's  */
{
	...
	for (i=0; i<nx; i++)
	{
		sum1 += z_cec2017[i]*z_cec2017[i];
		sum2 += cos(2.0*PI*z_cec2017[i]);
	}
...
void weierstrass_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Weierstrass's  */
{
	...
	sr_func (x, z_cec2017, nx, Os, Mr, 0.5/100.0, s_flag, r_flag); /* shift and rotate */
	...
	for (i=0; i<nx; i++)
	{
		...
		for (j=0; j<=k_max; j++)
		{
			sum += pow(a,j)*cos(2.0*PI*pow(b,j)*(z_cec2017[i]+0.5));
...
void griewank_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Griewank's  */
{
	...
	sr_func (x, z_cec2017, nx, Os, Mr, 600.0/100.0, s_flag, r_flag); /* shift and rotate */

	for (i=0; i<nx; i++)
	{
		s += z_cec2017[i]*z_cec2017[i];
		p *= cos(z_cec2017[i]/sqrt(1.0+i));
	}
	f[0] = 1.0 + s/4000.0 - p;
}
...
void rastrigin_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Rastrigin's  */
{
	...
	sr_func (x, z_cec2017, nx, Os, Mr, 5.12/100.0, s_flag, r_flag); /* shift and rotate */

	for (i=0; i<nx; i++)
	{
		f[0] += (z_cec2017[i]*z_cec2017[i] - 10.0*cos(2.0*PI*z_cec2017[i]) + 10.0);
	}
}
...
void step_rastrigin_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Noncontinuous Rastrigin's  */
{
	...
	for (i=0; i<nx; i++)
	{
		if (fabs(y_cec2017[i]-Os[i])>0.5)
		y_cec2017[i]=Os[i]+floor(2*(y_cec2017[i]-Os[i])+0.5)/2;
	}
...
void schwefel_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Schwefel's  */
{
	...
	sr_func (x, z_cec2017, nx, Os, Mr, 1000.0/100.0, s_flag, r_flag); /* shift and rotate */

	for (i=0; i<nx; i++)
	{
		z_cec2017[i] += 4.209687462275036e+002;
		if (z_cec2017[i]>500)
		{
			f[0]-=(500.0-fmod(z_cec2017[i],500))*sin(pow(500.0-fmod(z_cec2017[i],500),0.5));
			tmp=(z_cec2017[i]-500.0)/100;
			f[0]+= tmp*tmp/nx;
		}
		else if (z_cec2017[i]<-500)
		{
			f[0]-=(-500.0+fmod(fabs(z_cec2017[i]),500))*sin(pow(500.0-fmod(fabs(z_cec2017[i]),500),0.5));
			tmp=(z_cec2017[i]+500.0)/100;
			f[0]+= tmp*tmp/nx;
		}
		else
			f[0]-=z_cec2017[i]*sin(pow(fabs(z_cec2017[i]),0.5));
		}
...
void katsuura_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Katsuura  */
{
	...
	sr_func (x, z_cec2017, nx, Os, Mr, 5.0/100.0, s_flag, r_flag); /* shift and rotate */
...
void bi_rastrigin_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Lunacek Bi_rastrigin Function */
{
	...
	for (i=0; i<nx; i++)//shrink to the orginal search range
    {
        y_cec2017[i] *= 10.0/100.0;
    }

	for (i = 0; i < nx; i++)
    {
		tmpx[i]=2*y_cec2017[i];
        if (Os[i] < 0.0)
            tmpx[i] *= -1.;
    }
	for (i=0; i<nx; i++)
	...
	if (r_flag==1)
	{
		rotatefunc(z_cec2017, y_cec2017, nx, Mr);
		for (i=0; i<nx; i++)
		{
			tmp+=cos(2.0*PI*y_cec2017[i]);
		}	
		if(tmp1<tmp2)
			f[0] = tmp1;
		else
...
void grie_rosen_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Griewank-Rosenbrock  */
{
	...
	tmp1 = z_cec2017[nx-1]*z_cec2017[nx-1]-z_cec2017[0];
	tmp2 = z_cec2017[nx-1]-1.0;
    temp = 100.0*tmp1*tmp1 + tmp2*tmp2;
    f[0] += (temp*temp)/4000.0 - cos(temp) + 1.0 ;
}
...
void escaffer6_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Expanded Scaffer??s F6  */
{
    ...
    for (i=0; i<nx-1; i++)
    {
        temp1 = sin(sqrt(z_cec2017[i]*z_cec2017[i]+z_cec2017[i+1]*z_cec2017[i+1]));
		temp1 =temp1*temp1;
        temp2 = 1.0 + 0.001*(z_cec2017[i]*z_cec2017[i]+z_cec2017[i+1]*z_cec2017[i+1]);
        f[0] += 0.5 + (temp1-0.5)/(temp2*temp2);
    }
    temp1 = sin(sqrt(z_cec2017[nx-1]*z_cec2017[nx-1]+z_cec2017[0]*z_cec2017[0]));
	temp1 =temp1*temp1;
    temp2 = 1.0 + 0.001*(z_cec2017[nx-1]*z_cec2017[nx-1]+z_cec2017[0]*z_cec2017[0]);
    f[0] += 0.5 + (temp1-0.5)/(temp2*temp2);
}
...
/* original global optimum: [-1,-1,...,-1] */
{
    ...
    for (i=0; i<nx; i++)
    {
		z_cec2017[i]=z_cec2017[i]-1.0;//shift to orgin
        r2 += z_cec2017[i]*z_cec2017[i];
		sum_z_cec2017 += z_cec2017[i];
    }
    f[0]=pow(fabs(r2-nx),2*alpha) + (0.5*r2 + sum_z_cec2017)/nx + 0.5;
}
...
/* original global optimum: [-1,-1,...,-1] */
{
    ...
    f[0]=pow(fabs(pow(r2,2.0)-pow(sum_z_cec2017,2.0)),2*alpha) + (0.5*r2 + sum_z_cec2017)/nx + 0.5;
}
...
void hf01 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 1 */
{
	...
	for (i=0; i<nx; i++)
	{
		y_cec2017[i]=z_cec2017[S[i]-1];
	}
	i=0;
	zakharov_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=1;
	rosenbrock_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=2;
	rastrigin_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	f[0]=0.0;
	for(i=0;i<cf_num;i++)
	{
		f[0] += fit[i];
...
void hf02 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 2 */
{
	...
	i=0;
	ellips_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=1;
	schwefel_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=2;
	bent_cigar_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
...
void hf03 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 2 */
{
	...
	rosenbrock_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=2;
	bi_rastrigin_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
...
void hf04 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 3 */
{
	...
	ellips_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=1;
	ackley_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=2;
	schaffer_F7_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=3;
	rastrigin_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
...
void hf05 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 4 */
{
	...
	bent_cigar_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=1;
	hgbat_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=2;
	rastrigin_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=3;
	rosenbrock_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
...
void hf06 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 5 */
{
	...
	i=0;
	escaffer6_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=1;
	hgbat_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=2;
	rosenbrock_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
...
void hf07 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 6 */
{
	...
	i=0;
	katsuura_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=1;
	ackley_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=2;
	grie_rosen_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=3;
	schwefel_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=4;
	rastrigin_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
...
void hf08 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 6 */
{
	...
	hgbat_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=4;
	discus_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
...
void hf09 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 6 */
{
	...
	grie_rosen_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=3;
	weierstrass_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
	i=4;
	escaffer6_func(&y_cec2017[G[i]],&fit[i],G_nx[i],Os,Mr,0,0);
...
void sr_func (double *x, double *sr_x, int nx, double *Os,double *Mr, double sh_rate, int s_flag,int r_flag) /* shift and rotate */
{
	...
	if (s_flag==1)
	{
		...
		{	
			...
			{
				y_cec2017[i]=y_cec2017[i]*sh_rate;
			}
			rotatefunc(y_cec2017, sr_x, nx, Mr);
		}
		else
		{
